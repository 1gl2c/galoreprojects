...
test('disabled inquiry endpoint never returns success or echoes submitted text', async () => {
  const res = response();
  await quote({method:'POST',body:{email:'<script>private</script>'}}, res);
...
test('inquiry endpoint rejects unsupported methods', async () => {
  const res = response(); await quote({method:'GET'},res);
