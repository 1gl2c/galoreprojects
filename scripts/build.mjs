...
const siteOrigin = 'https://www.galoreprojects.com';
...
for (const page of ['index.html','work/index.html','qualifications/index.html']) {
  ...
  html=html.replace('</head>',`  <link rel="canonical" href="${siteOrigin}${route}">\n</head>`);
...
await writeFile(new URL('sitemap.xml',output),'<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'+['/','/work/','/qualifications/'].map(route=>`<url><loc>${siteOrigin}${route}</loc></url>`).join('')+'</urlset>\n');
